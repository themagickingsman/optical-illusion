...
import { drawSunLayers, DEFAULT_SUN_CONFIG } from './SunEnvironment';
...
function drawPlanet(ctx: CanvasRenderingContext2D, cx: number, cy: number, r: number, planet: any, vectorPayload?: any) {
  // Draw vector effects FIRST (so glows apply correctly underneath)
  if (vectorPayload && vectorPayload.sunConfig) {
      ctx.save();
      const cfg = { ...DEFAULT_SUN_CONFIG, ...vectorPayload.sunConfig };
      const pct = Math.max(0.001, cfg.sunSizePct);
      const W = (r * 100) / pct;
      drawSunLayers(ctx, W, W, cx, cy, cfg);
      ctx.restore();
      // Notice: Removed `return;` here so the standard planet draws on top
  }
...
export default function SolarSystemMapEngine({ 
  ...
  const vectorPresetsRef = useRef<any[]>([]);

  useEffect(() => {
     const updateVectors = () => {
        try { 
            const parsed = JSON.parse(localStorage.getItem('arn_splash_vectors_v1') || '[]'); 
            vectorPresetsRef.current = Array.isArray(parsed) ? parsed : [];
        } catch {
            vectorPresetsRef.current = [];
        }
     };
     updateVectors();
     const onStorage = (e: StorageEvent) => { if (e.key === 'arn_splash_vectors_v1') updateVectors(); };
  ...
  const draw = useCallback(() => {
    ...
    SOLAR_BODIES.filter(p => p.name !== 'Sun').forEach((planet, i) => {
      ...
      const vec = (Array.isArray(vectorPresetsRef.current) ? vectorPresetsRef.current : []).find(
          (v: any) => v && v.targetNode && typeof v.targetNode === 'string' && typeof planet.name === 'string' &&
          v.targetNode.toLowerCase() === planet.name.toLowerCase()
      );
      drawPlanet(ctx, px, py, ps, planet, vec);
  ...
  const getPlanetPositions = useCallback(() => {
    ...
    const yRatio = viewMode === 'topdown' ? 1.0 : 0.38;
    const t=Date.now();
    const prefs = mapPrefsRef?.current || { mapSpeed: 1.0 };
    
    return SOLAR_BODIES.filter((p: any) => p.name !== 'Sun').map((planet: any, i: number)=>{
      const angle = planet.baseAngle + (t * planet.orbSpeed) * (prefs.mapSpeed ?? 1.0) / 16.67; 
      const r = planet.worldR * mapScale;
      return { planet, x:cx+Math.cos(angle)*r, y:cy+Math.sin(angle)*r*yRatio, r:Math.max(10,(planet.r / 10)*actualRenderZoom*1.5) };
    });
  },[viewMode, visualZoomRef]);
