...
export default function PlanetManager({ onSelectPlanet, onClose }: PlanetManagerProps) {
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (awp) {
          const target = solarSystemBodies.find(b => b.name === awp);
    ...
    >
      ...
      <svg width="100%" height="100%" style={{ position: "absolute", inset: 0, overflow: "visible" }}>
        <g transform={`translate(${dim.w / 2 + cam.x * cam.scale}, ${dim.h / 2 + cam.y * cam.scale}) scale(${cam.scale})`}>
          ...
          {activeWaypoint && solarSystemBodies.find(b => b.name === activeWaypoint) && (() => {
             const target = solarSystemBodies.find(b => b.name === activeWaypoint)!;
             ...
             return (
               <line 
                 ...
                 strokeDasharray={`${8 / cam.scale} ${4 / cam.scale}`} 
                 opacity={0.8}
               />
             );
          })()}
            ...
            return (
              <g key={`planet-${i}`} transform={`translate(${px}, ${py})`}>
                <circle 
                  ...
                  fill={activeWaypoint === b.name ? '#ffffff' : b.color}
                  stroke={activeWaypoint === b.name ? '#00ffcc' : b.color}
                  strokeOpacity={activeWaypoint === b.name ? 1.0 : 0.5}
                  strokeWidth={activeWaypoint === b.name ? 8 / cam.scale : 4 / cam.scale}
