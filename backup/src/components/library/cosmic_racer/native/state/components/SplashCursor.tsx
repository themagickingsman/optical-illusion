...
export interface SplashCursorProps {
  ...
  externalEmits?: { id: string; x: number; y: number; dx?: number; dy?: number; color?: {r:number,g:number,b:number}, splatRadius?: number }[];
  externalEmitsRef?: React.MutableRefObject<any[]>;
  cameraDeltaRef?: React.MutableRefObject<{x: number, y: number}>;
  onSplat?: () => void;
  canvasForwardRef?: React.MutableRefObject<HTMLCanvasElement | null>;
...
export function SplashCursor({
  ...
  CURL = 3,
  SPLAT_RADIUS = 0.2, // Default react bits value
  SPLAT_FORCE = 6000,
  SHADING = true,
  COLOR_UPDATE_SPEED = 10,
  BACK_COLOR = { r: 0.0, g: 0, b: 0 },
  TRANSPARENT = true,
  fluidColor, // Using this to override random colors if provided
  externalEmits,
  externalEmitsRef,
  cameraDeltaRef,
  onSplat,
  ...
  const canvasRef = canvasForwardRef || localCanvasRef;
  const animationFrameId = useRef<number | null>(null);
  
  // Track externalEmits changes without resetting the whole WebGL context
  const activeEmitsRef = useRef(externalEmits || []);
  useEffect(() => {
    activeEmitsRef.current = externalEmits || [];
  }, [externalEmits]);

  // Keep all physics parameters synced dynamically
  ...
  useEffect(() => {
    ...
    let shipPointers: Record<string, any> = {};
    let wasExternalEmitActive = false;

    const webglCtx = getWebGLContext(canvas);
    if (!webglCtx || !webglCtx.gl || !webglCtx.ext) {
      console.warn('[SplashCursor] WebGL context creation failed. WebGL may not be supported or hardware acceleration is disabled.');
      return;
    }
    ...
    const displayShaderSource = `
      ...
      void main () {
          ...
          // Removing artificial edge masks so fluid renders cleanly up to the screen boundary
          // float edgeMaskX = smoothstep(0.0, 0.05, vUv.x) * smoothstep(1.0, 0.95, vUv.x);
          // float edgeMaskY = smoothstep(0.0, 0.05, vUv.y) * smoothstep(1.0, 0.95, vUv.y);
          // c *= (edgeMaskX * edgeMaskY);
          // a *= (edgeMaskX * edgeMaskY);
    ...
    const advectionShader = compileShader(
      ...
      `
        ...
        void main () {
                ...
                vec2 coord = vUv - dt * bilerp(uVelocity, vUv, texelSize).xy * texelSize + uCameraDelta;
                vec4 result = bilerp(uSource, coord, dyeTexelSize);
            #else
                vec2 coord = vUv - dt * texture2D(uVelocity, vUv).xy * texelSize + uCameraDelta;
    ...
    const divergenceShader = compileShader(
      ...
      `
        ...
        void main () {
            ...
            // Removed reflexive wall boundary conditions to allow fluid to flow freely off-screen
