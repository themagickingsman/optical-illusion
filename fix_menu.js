...
const targetPath = path.join(__dirname, 'src/components/library/TerrainGenerator.tsx');
let content = fs.readFileSync(targetPath, 'utf8');

// 1. Fix fetch URL unicode escape error
content = content.replace('await fetch(`/api/cms?key=${LS_KEY}\\u0026t=${Date.now()}`);', 'await fetch(`/api/cms?key=${LS_KEY}&t=${Date.now()}`);');
content = content.replace('await fetch(`/api/cms?key=${LS_KEY}\u0026t=${Date.now()}`);', 'await fetch(`/api/cms?key=${LS_KEY}&t=${Date.now()}`);');
...
console.log("Reverted UI and fixed Unicode successfully.");
