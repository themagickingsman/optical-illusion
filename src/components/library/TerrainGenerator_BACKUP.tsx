...
const DEFAULT_CITY_PRESET = {
  ...
  keyLightColor: '#c8aaff', lightElev: 13, lightAzimuth: 0, camElev: 35.26, camAzimuth: 45, camZoom: 56,
...
const DEFAULT_REGIONAL_PRESET = {
  ...
  keyLightColor: '#b8e8c0', lightElev: 48, lightAzimuth: 55, camElev: 35.26, camAzimuth: 45, camZoom: 72,
...
export default function TerrainGenerator({ lsKey: lsKeyProp, onClose, onStartExit, onLoadComplete }: { lsKey?: string, onClose?: () => void, onStartExit?: () => void, onLoadComplete?: () => void } = {}) {
  ...
  const _defaultPreset =
    ...
    LS_KEY === 'arn_terrain_city_v1'     ? DEFAULT_CITY_PRESET     : {
      ...
      keyLightColor: '#ffffff', lightElev: 37, lightAzimuth: 43, camElev: 35.26, camAzimuth: 45, camZoom: 56,
      terrainTint: '#1e7a8c', beaconCount: 0, beaconColor: '#ff6b35', beaconEmissive: 0.5,
      beaconLight: 0, beaconBury: 0, beaconSeed: 7, renderMode: 'glass',
  ...
  const recreateParticleSystem = (engine: RenderEngine, dim: '2D' | '3D') => {
      if (particleSystemRef.current && sceneRef.current) {
          particleSystemRef.current.dispose();
          sceneRef.current.remove(particleSystemRef.current.group);
      }
      if (engine === 'multimesh') particleSystemRef.current = new MultiMeshParticleSystem(dim);
  ...
  const announceWave = useCallback((wave: number, count: number, weaponUnlock?: string) => {
      ...
      if (el) {
          ...
          (async () => {
              ...
              if (cameraRef.current) cinematicZoomRef.current = cameraRef.current.zoom * 1.35;
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        }, 1200); // Artificial delay to ensure thin loader is visible
      }
    };
    load();
  }, [LS_KEY]);
  ...
  const spawnBlockExplosion = (px: number, py: number, pz: number, color: THREE.Color, count: number, speedMult: number, lifeMult: number, forceVel?: THREE.Vector3[], forcePos?: THREE.Vector3[], weaponRadius: number = 1) => {
    if (particleSystemRef.current) {
        ...
        particleSystemRef.current.explode(px, py, pz, force, 5.0, color, isTreasureModeRef.current, calculatedShards);
    }
  };
  ...
  useEffect(() => {
    ...
    particleSystemRef.current = new MultiMeshParticleSystem('2D');
        ...
        <div style={{ 
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
               <button 
                  onClick={() => { setParticleDimensionType('2D'); recreateParticleSystem(renderEngine, '2D'); }}
               ...
               <button 
                  onClick={() => { setParticleDimensionType('3D'); recreateParticleSystem(renderEngine, '3D'); }}
            ...
            <button 
               onClick={() => {
                  ...
                  recreateParticleSystem('multimesh', particleDimensionType);
               }}
            ...
            <button 
               onClick={() => {
                  ...
                  recreateParticleSystem('global', particleDimensionType);
               }}
            ...
            <button 
               onClick={() => {
                  setRenderEngine('gpu');
                  recreateParticleSystem('gpu', particleDimensionType);
               }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, flex: 1, overflowY: 'auto', paddingRight: 4, paddingBottom: 60 }}>
          ...
          {(layoutTab === 'procedural' || layoutTab === 'large_map') && (<>
            ...
            <div className="apple-card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                <CfgSlider label="Bloom Strength"  min={0} max={2.0} step={0.05} value={bloomStr}    baseline={_defaultPreset.bloomStr} onChange={setBloomStr}    accent="#8b5cf6" />
                <CfgSlider label="Bloom Threshold" min={0} max={1.0} step={0.05} value={bloomThresh} baseline={_defaultPreset.bloomThresh} onChange={setBloomThresh} accent="#8b5cf6" />
                <CfgSlider label="Block Glow Int." min={0} max={2.0} step={0.05} value={glowInt}     baseline={_defaultPreset.glowInt} onChange={setGlowInt}     accent="#8b5cf6" />
            ...
            <div className="apple-card">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <CfgSlider label="Zoom"       min={8}   max={120}  step={1}    value={camZoom}    onChange={setCamZoom}    accent="#67e8f9" />
              </div>
            </div>
