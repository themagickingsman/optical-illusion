...
const LS_KEY = 'arn_glowing_sun_config';
...
export default function GlowingSunStudio({ onBack }: { onBack?: () => void }) {
  const [cfg, setCfg] = useState<GlowingSunConfig>(() => {
    if (typeof window === 'undefined') return GLOWING_SUN_DEFAULTS;
    try {
      const raw = localStorage.getItem(LS_KEY);
      if (raw) return { ...GLOWING_SUN_DEFAULTS, ...JSON.parse(raw) };
    } catch {}
    return GLOWING_SUN_DEFAULTS;
  });
