...
export default function SolarSystemMapEngine({ 
  ...
  const draw = useCallback(() => {
    ...
    ctx.setLineDash([]); // Reset line dash to prevent fleet marker dashes from leaking into orbital rings
    ctx.lineDashOffset = 0;

    // Orbital rings — identical geometric paths as AsteroidsGame
    SOLAR_BODIES.forEach(planet => {
      const r = planet.worldR * mapScale;
      ctx.beginPath();
      ctx.ellipse(cx, cy, r, r * yRatio, 0, 0, Math.PI*2);
      ctx.strokeStyle = planet.color ? planet.color + '33' : 'rgba(255,255,255,0.2)';
      ctx.lineWidth = 1.2;
      ctx.stroke();
    });
    ...
    SOLAR_BODIES.forEach((planet, i) => {
      ...
      const fontSize = Math.max(9, 12 * labelScale);
  ...
  const getPlanetPositions = useCallback(() => {
    ...
    const yRatio = viewMode === 'topdown' ? 1.0 : 0.38;
    const t=Date.now();
    return SOLAR_BODIES.map((planet,i)=>{
      const angle = planet.baseAngle + (t * planet.orbSpeed) / 16.67; 
      const r = planet.worldR * mapScale;
    ...
    const loop=()=>{
      ...
      if(!isDragging.current){
         ...
         if (targetPanX !== null && targetPanY !== null) {
           ...
           if (Math.abs(panRef.current.x) < 1 && Math.abs(panRef.current.y) < 1) {
  ...
  const onMove=(e:React.PointerEvent)=>{
    ...
    if(e.buttons>0) {
      ...
      setHoveredPlanet(hit?.type==='planet'?(hit.target as any).name:null);
      setHoveredShipId(hit?.type==='ship'?(hit.target as any).id:null);
    }
    lastPointer.current={x:e.clientX,y:e.clientY};
  };
  const onUp=(e:React.PointerEvent)=>{
    if(!isDragging.current){
      ...
      if (hit?.type === 'planet') {
         setSelectedShipId(null);
         const targetPlanet = hit.target as any;
         setSelectedPlanet((p: any)=>{
           if (p?.name !== targetPlanet.name) setIsInfoExpanded(false);
           const next = p?.name===targetPlanet.name?null:targetPlanet;
         ...
         const targetShip = hit.target as any;
         setSelectedShipId((id: any)=>{
            const next = id === targetShip.id ? null : targetShip.id;
            isReturningToSun.current = !next;
            return next;
         });
      } else {
