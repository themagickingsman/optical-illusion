  ...
  public update() {
    ...
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      ...
      if (p.type === 'artillery' && p.curvePt1 && p.curvePt2) {
        ...
        p.mesh.scale.x = 1.0 + Math.sin(now * 0.6) * 0.03;
        p.mesh.scale.z = 1.0 + Math.sin(now * 0.6) * 0.03;
        (p.mesh.material as THREE.MeshBasicMaterial).opacity = 0.8 + Math.sin(now * 0.8) * 0.2;
