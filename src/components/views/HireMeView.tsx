...
export default function HireMeView() {
  ...
  // Initial Tutorial Greeting
  const [localMessages, setLocalMessages] = useState<any[]>([
    { id: 't1', sender: 'admin', text: 'Welcome to Optical Illusions.', timestamp: new Date().toISOString() },
    { id: 't2', sender: 'admin', text: 'I\'m available for Front-end design + dev. React, Three.js, Unreal, Unity.', timestamp: new Date().toISOString() },
    { id: 't3', sender: 'admin', text: 'Per month billing, daily syncs, daily deliverables. 2 weeks lead time to start.', timestamp: new Date().toISOString() },
    { id: 't4', sender: 'admin', text: 'Tell me a little bit about your project and when you\'d like to start.', timestamp: new Date().toISOString() }
  ]);
  ...
  return (
    <div style={{ width: "100%", maxWidth: "1280px", height: "650px", display: "flex", gap: "20px", color: "#fff", transform: "scale(0.85)", transformOrigin: "center center", fontFamily: "var(--font-rubik)", fontWeight: 500 }}>
      
      {/* Left Pane: Game Mosaic Grid (Replaced NDA Form) */}
      <GameMosaic />

      {/* Right Wrapper */}
      <div style={{ flex: 1.5, position: "relative", display: "flex" }}>
        
        {/* Floating Social Pill Buttons */}
        <div style={{ position: "absolute", top: "-55px", right: "0", display: "flex", gap: "12px" }}>
        ...
        <div className="glass-panel" style={{ flex: 1, width: "100%", background: "rgba(255, 255, 255, 0.03)", backdropFilter: "blur(100px)", WebkitBackdropFilter: "blur(100px)", borderRadius: "20px", border: "1px solid rgba(255,255,255,0.1)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "40px", display: "flex", flexDirection: "column", gap: "15px" }}>
          {allMessages.map((msg, idx) => (
            <div key={msg.id || idx} style={{
              ...
              <div style={{ fontSize: '14px', color: '#0ff', marginTop: '8px', textAlign: msg.sender === 'user' ? 'right' : 'left' }}>
        ...
        <div style={{ padding: "25px 40px 50px 40px", borderTop: "1px solid rgba(255,255,255,0.1)" }}>
          <div style={{ display: "flex", gap: "10px", background: "rgba(255,255,255,0.05)", padding: "5px", borderRadius: "100px", border: "1px solid rgba(255,255,255,0.1)" }}>
            <input
              type="text"
              value={inputText}
              ...
              style={{ flex: 1, background: "transparent", border: "none", color: "#fff", fontSize: "22px", padding: "20px 28px", outline: "none" }}
            />
            <button
              onClick={handleSendMessage}
              style={{ background: "#0A84FF", color: "#fff", border: "none", borderRadius: "100px", padding: "0 45px", fontSize: "22px", fontWeight: "bold", cursor: "pointer", transition: "transform 0.1s" }}
